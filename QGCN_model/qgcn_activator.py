...
class QGCNActivator:
    ...
    def _load_data(self, train_dataset, train_siza, dev_size, test_size, batch_size):
        # calculate lengths of train and dev according to split ~ (0,1)
    ...
    def train(self, show_plot=True, early_stop=False):
        ...
        for epoch_num in range(self._epochs):
            ...
            for batch_index, (A, x0, embed, label) in enumerate(self._train_loader):
                list_Ax.append(torch.matmul(A, x0).numpy())
                if self._gpu:
                ...
                loss = self._loss_func(output, label.unsqueeze(dim=1).float(), weight=torch.Tensor([self._loss_weights[i].item() for i in label]).unsqueeze(dim=1))\
                    if self._is_binary else self._loss_func(output, label, weight=self._loss_weights)
                # calculate loss
